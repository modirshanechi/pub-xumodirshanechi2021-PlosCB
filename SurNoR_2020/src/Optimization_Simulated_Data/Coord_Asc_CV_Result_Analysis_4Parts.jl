# The code for organizing the optimization results separately for each of the
# 4 repetitions of 3-fold cross-validation and saving them in data
using CSV
using DataFrames
using Query
using PyPlot
    ...
    for Alg_ind = 1:length(Alg_Path)
        ...
        for Fold = 1:3
            ...
            Path = string(Path_Files, "CoAscData_", update_form, "_nset_", string(n_set), "_F", string(Fold), "_C",
                           string( (Part_ind-1)*Chains + Chain), ".csv")
            ...
            title(string(update_form, " - LogL Series, Fold ", string(Fold), ", Part", string(Part_ind) ))
            ax.grid(true)
            Path_save = "data/Fitted_parameters_simulated/CV_3Fold/"
            savefig(string(Path_save, update_form, " - LogL Series_nset_", string(n_set), "_Fold ", string(Fold), ", Part", string(Part_ind), ".pdf" ))
            close(fig)
            # ------------------------------------------------------------------------------
            # V_Cat
        ...
        Path = string(Path_save, "nset_", string(n_set), "/", update_form, "_BestParams", "_Part", string(Part_ind), ".csv" )
