...
for Update_Ind = 1:2
        update_form = Update_Forms[Update_Ind]
        ...
        for Chain = 1:100
            @distributed for Fold = 1:3
                    ...
                    Param_Seq = zeros(T_Iter, 9)                                                                            # WARNING: SHOULD CHANGE

                    Param_Seq[1,1] = 0.0 + 0.01*rand(1)[1]             # pc
                    Param_Seq[1,2] = 1e-6 + (9e-6)*rand(1)[1]          # ϵ
                    Param_Seq[1,3] = rand(Rayleigh(5) ,1)[1]           # β_R1
                    ...
                    Param_Seq[1,8] = rand(1:20)[1]                     # N_PS
                    Param_Seq[1,9] = rand(Rayleigh(5) ,1)[1]           # β_R2
                    ...
                    Param1.N_prior_sweep = Param_Seq[1,8]

                    Param2 = deepcopy(Param1)
                    Param2.β_ratio_N2R = Param_Seq[1,5]
                    Param2.β_R = Param_Seq[1,9]
                    ...
                    while (t<=T_Iter)&&Flag
                            ...
                            Param_perm_order = shuffle(1:9)                                                                    # WARNING: SHOULD CHANGE
                            for i = 1:length(Param_perm_order)
                                    ...
                                    for x_ind = 1:length(x)
                                            ...
                                            Param1.N_prior_sweep = Param_temp[8]

                                            Param2 = deepcopy(Param1)
                                            Param2.β_ratio_N2R = Param_temp[5]
                                            Param2.β_R = Param_temp[9]
                            ...
                            if (t==T_Iter)||(mod(t,5)==0)||(!Flag)                                                              # WARNING: SHOULD CHANGE
                                    Path = string("src/Optimization/CV_3Fold/CoAscData_MB_", update_form, "_F", string(Fold), "_C",
                                                    string(Chain), ".csv")
